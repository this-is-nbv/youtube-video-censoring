...
class LowRankFusionTA(nn.Module):
    ...
    def __init__(self, d_text, d_audio, d_out, rank=4):
        ...
        self.Wt = nn.Parameter(
            torch.randn(rank, d_text)
        )

        self.Wa = nn.Parameter(
            torch.randn(rank, d_audio)
        )

        self.Wo = nn.Parameter(
            torch.randn(rank, d_out)
        )
...
class MFusTSVD_TA(nn.Module):
    ...
    def __init__(self, d_model=256):
        ...
        self.self_text = SelfModalityTransformer(
            d_model
        )

        self.self_audio = SelfModalityTransformer(
            d_model
        )

        self.cross = CrossModalityTransformer(
            d_model
        )

        self.lrmf = LowRankFusionTA(
            d_model,
            d_model,
            d_model
        )

        self.classifier = MFusTSVDClassifier(
            d_model
        )
    ...
    def forward(self, text_feat, audio_feat):
        ...
        text_cross = self.cross(
            text_self,
            audio_self,
            audio_self
        )

        audio_cross = self.cross(
            audio_self,
            text_self,
            text_self
        )
